...
from example.kaeri.isaac.quadruped.go1.controller.go1_qp_control import Go1QPControl

import os


class Go1ControlBase:
    """[summary]
    ...
    def __init__(self, name: str, _dt: float) -> None:
        """Initialize the QP Controller.

        Args:
            name {str} -- The name of the layer.
            _simulated_dt {float} -- rough estimation of the time interval of the control loop
            _control_model {str} -- The name of the algorithm used for control. Default is None.
        ...
        # robot controller
        self._qp_control = Go1QPControl() # QP
    ...
    def advance(self, dt: float, measurement: A1Measurement) -> np.array:
        if self._control_model == "RL":
            return self.advance_rl(dt, measurement)
        else:
            return self.advance_qp(dt, measurement)
        
    def advance_rl(self, dt: float, measurement: A1Measurement) -> np.array:
        ...
        # update controller states from target command (only cmd_vel is needed)
        self._desired_states._root_lin_vel_d[0] = self._current_base_command[0] # vx
        self._desired_states._root_lin_vel_d[1] = self._current_base_command[1] # vy
        self._desired_states._root_ang_vel_d[2] = self._current_base_command[2] # vtheta

        self._rl_control.update_plan(self._desired_states, self._ctrl_states, self._ctrl_params, dt)

    def advance_qp(self, dt: float, measurement: A1Measurement) -> np.array:
        """[summary]
        ...
        # update controller states from target command (cmd_vel)
        ...
        self._qp_control.update_plan(self._desired_states, self._ctrl_states, self._ctrl_params, dt)
        # update_plan updates swing foot target
        # swing foot control and stance foot control
        torques = self._qp_control.generate_ctrl(self._desired_states, self._ctrl_states, self._ctrl_params)
        return torques
    ...
    def ctrl_state_reset(self) -> None:
        ...
        self._ctrl_params = A1CtrlParams()
        self._ctrl_params._kp_linear = np.array([500, 500.0, 160.0])
