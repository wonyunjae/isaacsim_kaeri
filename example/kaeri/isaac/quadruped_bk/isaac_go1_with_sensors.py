...
# custom omnigraph
from example.kaeri.quadruped.omnigraph.ros_publisher import *
...
class Go1_runner(object):
    def __init__(self) -> None:
        ...
        # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= [config] =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        # load yaml file
        with open(PACKAGE_PATH + "/example/kaeri/quadruped/config/config_isaac.yaml", "r") as f:
        ...
        # Get go1
        go1_prim = stage.GetPrimAtPath("/World/Robot/Go1")
