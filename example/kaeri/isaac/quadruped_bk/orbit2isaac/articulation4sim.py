...
from omni.isaac.orbit.assets.rigid_object import RigidObject
from omni.isaac.orbit.assets.articulation.articulation_data import ArticulationData
...
class Articulation4SIM(RigidObject):
    ...
    def update(self, dt: float | None = None):
        ...
        self._data.root_state_w[:, :7] = torch.from_numpy(self.root_physx_view.get_root_transforms())
        self._data.root_state_w[:, 3:7] = math_utils.convert_quat(self._data.root_state_w[:, 3:7], to="wxyz")
        self._data.root_state_w[:, 7:] = torch.from_numpy(self.root_physx_view.get_root_velocities())
        # -- joint states
        self._data.joint_pos[:] = torch.from_numpy(self.root_physx_view.get_dof_positions())
        self._data.joint_vel[:] = torch.from_numpy(self.root_physx_view.get_dof_velocities())
        self._data.joint_acc[:] = (self._data.joint_vel - self._previous_joint_vel) / dt
        # -- update common data
        # note: these are computed in the base class
        self._update_common_data_overriding(dt)
        # -- update history buffers
        self._previous_joint_vel[:] = self._data.joint_vel[:]

    def _update_common_data_overriding(self, dt: float):
        # -- body-state (note: we roll the quaternion to match the convention used in Isaac Sim -- wxyz)
        self._data.body_state_w[..., :7] = torch.from_numpy(self.body_physx_view.get_transforms()).view(-1, self.go1_link_num, 7)
        self._data.body_state_w[..., 3:7] = math_utils.convert_quat(self._data.body_state_w[..., 3:7], to="wxyz")
        self._data.body_state_w[..., 7:] = torch.from_numpy(self.body_physx_view.get_velocities()).view(-1, self.go1_link_num, 6)
        # -- body acceleration
    ...
    def write_joint_effort_limit_to_sim(
        ...
        if isinstance(limits, torch.Tensor):
            limits = limits.cpu()
        # set into internal buffers
        torque_limit_all = torch.from_numpy(self.root_physx_view.get_dof_max_forces())
    ...
    def _create_buffers(self):
        ...
        joint_pos_limits = torch.from_numpy(self.root_physx_view.get_dof_limits())
    ...
    def _process_actuators_cfg(self):
        ...
        usd_stiffness = torch.from_numpy(self.root_physx_view.get_dof_stiffnesses()).clone()
        usd_damping = torch.from_numpy(self.root_physx_view.get_dof_dampings()).clone()
        usd_armature = torch.from_numpy(self.root_physx_view.get_dof_armatures()).clone()
        usd_friction = torch.from_numpy(self.root_physx_view.get_dof_friction_coefficients()).clone()
        usd_effort_limit = torch.from_numpy(self.root_physx_view.get_dof_max_forces()).clone()
        usd_velocity_limit = torch.from_numpy(self.root_physx_view.get_dof_max_velocities()).clone()
        ...
        for actuator_name, actuator_cfg in self.cfg.actuators.items():
            ...
            actuator.network.to('cpu')
            carb.log_info(
                f"actuator net inferenced in cpu by wontae"
            )
    ...
    def _apply_actuator_model(self):
        ...
        for actuator in self.actuators.values():
            ...
            if control_action.joint_efforts is not None:
                self._joint_effort_target_sim[:, actuator.joint_indices] = control_action.joint_efforts.detach()
